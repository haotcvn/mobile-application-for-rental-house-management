...
import com.tranconghaoit.homemanager.databinding.FragmentPaidBinding
...
class PaidFragment : Fragment() {
    ...
    private val binding: FragmentPaidBinding get() = _binding!!
    ...
    override fun onCreateView(
        ...
        return FragmentPaidBinding.inflate(inflater, container, false).also {
            _binding = it
        }.root
    }
    ...
    private fun performGetBill() {
        if (NetworkUtils.haveNetworkConnection(requireContext())) {
            ...
            RetrofitClient.apiService.getBillPaid(userID)
    ...
    fun filterData(query: String) {
        ...
        for (item in originalList) {
            val billID = item.billID.toString().lowercase(Locale.ROOT).contains(query.lowercase(
                Locale.ROOT))
